...
import {
  ChartExport,
} from "../../modules/ChartExporter.module";
...
describe("ChartExporter", function () {
  const testChartValidNoMath: ChartModel = {
    name: "fake_chart",
    type: "line",
    slug: "no-math-slug",
    category: "fake",
  ...
  const testChartValidAddition: ChartModel = {
    ...
    it("should return a chart model with data in its data array", function (done) {
      ...
      chart
        ...
        .then((chart) => {
          ...
          assert.equal(
            chart.data.length,
            testChartValidNoMath.variables.length
          );
    ...
    it("should return a chart model with a single array of data that is the result of two summed variables", function (done) {
      ...
      chart
        ...
        .then((chart) => {
          ...
          assert.equal(
            chart.data.length,
            testChartValidAddition.variables.length
          );
          chart.data.forEach((group) => assert(group.data.length > 0));
          chart.data.forEach((group) =>
            group.data.forEach((datum) => assert(parseFloat(datum.value) > 99))
          );
          done();
        })
        .catch((err) => done(err));
    ...
    it("should return a chart export with cut values", function (done) {
      ...
      chart
        ...
        .then((chart) => {
          ...
          chart.data.forEach((group) =>
            group.data.forEach((datum) => assert(datum.value < 1))
          );
          done();
        })
        .catch((err) => done(err));
  ...
  describe("#calculate inflation", function () {
    it("should return adjusted numbers", function (done) {
      let exporter = new ChartExport(nwData, testChartValidNoMath, {
        ...
        .then((response) => {
          ...
          response.data.forEach((datum) =>
            datum.data.forEach((item) =>
              expect(item.value).to.be.greaterThan(70)
            )
          );
          done();
        })
    ...
    ChartSchema.find({
      name: {
        $in: [
          "fake_chart",
          "bad_chart",
          "valid_addition_chart",
          "fake_chart_with_optional_var",
        ],
      },
    })
      .remove()
  ...
  after("remove test org and variables", function (done) {
    VariableDefinitionSchema.find({
      variable: {
        $in: ["test_var_1", "test_var_2", "test_var_3", "test_var_4"],
      },
    })
      .remove()
      .exec()
      .then(() =>
        SchoolSchema.find({
          unitid: { $in: [nwData.unitid, nwDataSector6.unitid] },
        })
          .remove()
          .exec()
      )
      .then(() =>
        SchoolDataSchema.find({
          unitid: { $in: [nwData.unitid, nwDataSector6.unitid] },
        })
          .remove()
          .exec()
