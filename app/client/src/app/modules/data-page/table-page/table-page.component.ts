...
import {
  SchoolDataSourceAgg,
  VarDataSourceExport,
} from "../../../services/SchoolDataSource/SchoolDataSource";
...
export class TablePageComponent implements OnInit {
  ...
  matTableDataSource = new MatTableDataSource<VarDataSourceExport>();
  selectedVariable: VariableDefinitionModel;
  showTable = false;
  ...
  _updateUI(args: any) {
    if (["sum", "avg"].includes(args.gbFunc)) {
      this.isAggregateForm.patchValue(
        { isAggregate: true },
        { emitEvent: false }
      );
    }
  ...
  getRightArrowVisible() {
    return (
      this.visibleColumns[this.visibleColumns.length - 1] !==
      this._columns[this._columns.length - 1]
    );
  }

  getShowSearchBox() {
    return (
      !!this.tableOptionsForm.get("variable").value &&
      !this.isAggregateForm.get("isAggregate").value
    );
  }

  getTableIsCurrency() {
    return this.selectedVariable
      ? /currency+./.test(this.selectedVariable.valueType)
      : false;
  }
  ...
  goToVariableSource() {
    this.router.navigate([
      "data/sources/variables/" + this.selectedVariable.variable,
    ]);
  }
  ...
  private _query(queryString: string) {
    ...
    if (!this.selectedVariable) {
      fetch.push(
        <any>this.varDefSelect.onVariableDefinitionSelect.pipe(first())
      );
    }

    observableForkJoin(...fetch)
      .pipe(
        map((res) => {
          return new SchoolDataSourceAgg(
            res[0],
            this.tableOptionsForm.value.gbField
          );
        }),
  ...
  setVisibleColumns() {
    let numCols =
      window.innerWidth > 600 ? (window.innerWidth > 1000 ? 5 : 3) : 1;
    this.visibleColumns = [];
  ...
  private _formatValues(data: VarDataSourceExport[]): VarDataSourceExport[] {
    return data.map((item: VarDataSourceExport) => {
      return _.forIn(item, (v, k, obj) => {
        if (_.toNumber(k) && _.toNumber(v)) {
          obj[k] = this.util
            .numberFormatter()
            .format(v, this.selectedVariable.valueType);
        }
      });
    });
  }
  ...
  private _onlyNameSearchChanged(
    prev: intTableOptionsForm,
    curr: intTableOptionsForm
  ): boolean {
    const formBezMatch = [prev, curr]
