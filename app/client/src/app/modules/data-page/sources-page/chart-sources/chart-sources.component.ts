...
export class ChartSourcesComponent implements OnInit {
  ...
  constructor(
    ...
    private vds: VariableDefinitions
  ) {}
  ...
  private _initializeDefinitions() {
    const definitions = this.chart.variables.map((variable) => {
      ...
      uniqueSymbolNodes = _.uniq(
        _.flatMap(definitions, (definition) => definition.variables)
      );
    this.vds
      .fetchByName(uniqueSymbolNodes.join(","))
      .pipe(first())
      .subscribe((defs) => {
        definitions.forEach((def) => {
          let matches: VariableDefinitionModel[] = [];
          defs.forEach((df) => {
...
export interface intChartSourceDefinition {
  ...
  formula: string;
  variables: string[];
  variableDefinitions: VariableDefinitionModel[];
}
