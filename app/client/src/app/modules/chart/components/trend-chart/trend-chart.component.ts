...
export class TrendChartComponent {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      ...
      changes.chartData.currentValue.chart
    ) {
  ...
  onChartDataChanges(chartDataChanges) {
    ...
    if (
      !chartDataChanges.currentValue.data.some((datum) => datum.data.length > 0)
    ) {
      if (this.chart) {
        this.chart.chartData = new ChartData(
          chartDataChanges.currentValue.data
        );
        this.chart.remove();
      }
      chartEmpty = true;
    ...
    //if chart or school is new, fetch new chart
      ...
      !chartDataChanges.previousValue ||
      !_.isEqual(
        chartDataChanges.previousValue.chart,
        chartDataChanges.currentValue.chart
      ) ||
      !_.isEqual(
        chartDataChanges.previousValue.school,
        chartDataChanges.currentValue.school
      )
    ) {
      if (this.chart) this.chart.remove();
      this.chart = this.Charts.resolveChart(
        chartDataChanges.currentValue,
        this.myRandomSelector,
        this.chartOverrides
      );
    }
    //if only the data changed, redraw current chart
      ...
      !_.isEqual(
        chartDataChanges.previousValue.data,
        chartDataChanges.currentValue.data
      )
    ) {
