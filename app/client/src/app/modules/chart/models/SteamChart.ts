...
export class SteamChart extends BaseChart {
  ...
  build() {
    ...
    this.area = d3
      ...
      .curve(d3.curveStep)
      .x(function (d: any, i) {
        return that.xScale(d.data.date);
      })
      .y0(function (d) {
    ...
    this.stack
      .keys(this.chartData.data.map((datum) => datum.key))
  ...
  draw() {
    ...
    this.yScale.domain([
      d3.min(stackData, stackMin),
      d3.max(stackData, stackMax),
    ]);
    ...
    let paths = this.canvas.selectAll(".steam-path").data(stackData);
    ...
    paths.transition().duration(300).attr("d", this.area).style("opacity", 1);

    let enteredPaths = paths
      .enter()
      .append("path")
      .attr("class", "steam-path")
      .attr("d", this.area)
      .style("fill", function (d, i) {
        return that.zScale(i);
      })
      .style("opacity", 0);
