...
export class BubbleStackChart extends BaseChart {
  ...
  draw() {
    ...
    this.rScale.domain([this.chartData.getMin(), this.chartData.getMax()]);
    ...
    this.chartData.data.forEach((item) =>
      item.data.forEach(
        (datum: any) => (datum.rValue = this.rScale(datum.value))
      )
    );
    ...
    const circlesWithData = circles.data(
      this.chartData.data,
      (d: any) => d.key
    );
    const removedCircles = circlesWithData.exit().remove();
    const enteredCircles = circlesWithData
      .enter()
      .append("g")
      .attr("class", "layer");
    ...
    const legendData = this.chartData.data
      .sort((a, b) => {
        return d3.max(a.data, (datum) => datum.value) >
          d3.max(b.data, (datum) => datum.value)
          ? -1
          : 1;
      })
