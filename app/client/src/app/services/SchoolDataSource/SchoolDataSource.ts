...
export interface VarDataSourceExport {
  fiscal_year: string;
  [key: string]: any;
}
...
export class SchoolDataSourceAgg {
  ...
  private data: VarDataSourceExport[];

  constructor(_data: ExportAgg, keyCol: string) {
    this._keyCol = keyCol;
  ...
  _transformExport(_data): VarDataSourceExport[] {
    ...
    let exportData = <VarDataSourceExport[]>(
      _.flatMap(data, (datum) =>
        datum.reduce(
          (a, b) => Object.assign(a, { [b.fiscal_year]: b.value }),
          {}
        )
      )
    );
    ...
    if (this._keyCol === "sector") {
      keyMap = keyMap.map(
        (key) => sectors.find((sector) => sector.number === key).name
      );
    }
  ...
  _fillInMissingYears(data: intermediateData[][], variable: string) {
    const yearRange = d3
      .timeYears(new Date("2001"), new Date())
      .map((year) => year.getFullYear())
      .map((year) => String(year));

    data.forEach((datum) => {
      _getMissingYears(datum).forEach((fiscal_year) =>
        datum.push({
